<div class="modal fade" id="actualizarInfoModal" tabindex="-1" aria-labelledby="actualizarInfoModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form method="POST" action="{{ route('productos_consumibles.actualizarInfo', $productoConsumible->id) }}" class="needs-validation" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control shadow-sm" id="codigoBarraInput" name="codigoBarra"
                                    ...
                                    oninput="renderBarcodeVisual('#barcode-preview-consumible-edit', this.value); document.getElementById('preview-box-consumible-edit').classList.toggle('d-none', !this.value.trim());">
                                <button class="btn btn-outline-primary" type="button" onclick="generarCodigoBarra('codigoBarraInput', 'barcode-preview-consumible-edit'); document.getElementById('preview-box-consumible-edit').classList.remove('d-none');" title="Autogenerar código de barras único">
                                    <i class="fas fa-magic me-1"></i> Generar
                                </button>
                            </div>
                            <div id="preview-box-consumible-edit" class="barcode-auto-preview mt-2 p-2 bg-white rounded border text-center {{ $producto->codigoBarra ? '' : 'd-none' }} shadow-xs">
                                <small class="text-muted d-block mb-1">Previsualización:</small>
                                <svg id="barcode-preview-consumible-edit" class="img-fluid" style="max-height: 45px;"></svg>
                            </div>
                            @if($producto->codigoBarra)
                            <script>
                                document.addEventListener('DOMContentLoaded', function() {
                                    if (window.renderBarcodeVisual) {
                                        renderBarcodeVisual('#barcode-preview-consumible-edit', '{{ $producto->codigoBarra }}', { height: 35, fontSize: 11 });
                                    }
                                });
                            </script>
                            @endif
                        ...
                        <div class="col-md-6">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                ...
                                <div class="form-check form-check-inline mb-0 me-0">
                                    <input class="form-check-input" type="checkbox" id="no_aplica_precio_cons_edit" 
                                           {{ is_null($producto->precio) ? 'checked' : '' }}
                                           onchange="toggleNoAplicaPrecioConsEdit(this, 'precioInput')">
                                    <label class="form-check-label text-muted small" for="no_aplica_precio_cons_edit">No aplica</label>
                                </div>
                            </div>
                            <div class="input-group">
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control shadow-sm" id="comentarioTextarea" name="comentario"
                                rows="2" placeholder="Ingrese cualquier comentario relevante sobre el producto">{{ old('comentario', $productoConsumible->comentario) }}</textarea>
                        </div>
                    </div>
                </div>
...
<script>
function toggleNoAplicaPrecioConsEdit(checkbox, inputId) {
